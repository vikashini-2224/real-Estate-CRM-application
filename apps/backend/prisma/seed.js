...
async function main() {
  ...
  console.log("✅ Created users:", {
    admin: admin.email,
    agentJohn: agentJohn.email,
    agentSarah: agentSarah.email,
  });
  ...
  const skyviewHeights = await prisma.project.create({
    data: {
      ...
      description:
        "Ultra-luxury high-rise residential towers featuring panoramic skyline views and 5-star amenities.",
    },
  });
  ...
  const oasisVillas = await prisma.project.create({
    data: {
      ...
      description:
        "Exclusive private gated community with sprawling contemporary villas, infinity pools, and golf course access.",
    },
  });
  ...
  const horizonBay = await prisma.project.create({
    data: {
      ...
      description:
        "Modern coastal waterfront apartments with private yacht slips, spa, and rooftop sunset decks.",
    },
  });
  ...
  console.log(
    `✅ Created ${createdUnits.length} property units across 3 projects.`,
  );
  ...
  const lead2 = await prisma.lead.create({
    data: {
      ...
      budget: 35000000,
      requirement:
        "Interested in Skyview Penthouse or California Villa for family retreat.",
  ...
  const lead4 = await prisma.lead.create({
    data: {
      ...
      budget: 7500000,
      requirement:
        "First-time home buyer looking for 1BHK in Downtown or Miami Bayfront.",
  ...
  await prisma.note.createMany({
    data: [
      {
        content:
          "Initial intake call completed. Robert prefers higher floors with morning sunlight.",
        leadId: lead1.id,
        authorId: agentJohn.id,
      },
      {
        content:
          "Scheduled private weekend site tour for Elena at Skyview PH-01 at 2:00 PM.",
        leadId: lead2.id,
        authorId: agentSarah.id,
      },
      {
        content:
          "Elena loved the panoramic skyline terrace. Requested brochure for California Oasis Villas too.",
        leadId: lead2.id,
        authorId: agentSarah.id,
      },
      {
        content:
          "Client requested a 3% waiver on parking spot fees. Escalated to Admin for pricing approval.",
        leadId: lead3.id,
        authorId: agentJohn.id,
      },
      {
        content:
          "Booking contract signed and initial token payment of ₹15,00,000 received successfully.",
        leadId: lead5.id,
        authorId: agentJohn.id,
      },
    ],
