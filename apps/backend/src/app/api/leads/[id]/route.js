...
export const PATCH = withAuth(async (req, { params, user }) => {
  ...
  if (data.requirement !== undefined)
    updatePayload.requirement = data.requirement;
  ...
  if (data.interestedProjectId !== undefined)
    updatePayload.interestedProjectId = data.interestedProjectId || null;
  if (data.interestedBuildingId !== undefined)
    updatePayload.interestedBuildingId = data.interestedBuildingId || null;
  if (data.interestedUnitId !== undefined)
    updatePayload.interestedUnitId = data.interestedUnitId || null;
  ...
  if (updatePayload.interestedUnitId) {
    ...
    if (!unit)
      return NextResponse.json(
        { error: "Invalid interested unit" },
        { status: 400 },
      );
    if (
      updatePayload.interestedBuildingId &&
      updatePayload.interestedBuildingId !== unit.buildingId
    ) {
      return NextResponse.json(
        { error: "Unit does not belong to the selected building" },
        { status: 400 },
      );
    }
    if (
      updatePayload.interestedProjectId &&
      updatePayload.interestedProjectId !== unit.building.projectId
    ) {
      return NextResponse.json(
        { error: "Unit does not belong to the selected project" },
        { status: 400 },
      );
    }
    updatePayload.interestedBuildingId = unit.buildingId;
    ...
    if (!building)
      return NextResponse.json(
        { error: "Invalid interested building" },
        { status: 400 },
      );
    if (
      updatePayload.interestedProjectId &&
      ...
      return NextResponse.json(
        { error: "Building does not belong to the selected project" },
        { status: 400 },
      );
    }
    updatePayload.interestedProjectId = building.projectId;
  ...
  async (_req, { params }) => {
    ...
    if (lead.booking) {
      return NextResponse.json(
        {
          error:
            "Cannot delete lead with active booking. Cancel the booking first.",
        },
        { status: 400 },
      );
    }
