...
import { ProjectDTO, UnitDTO, UnitStatus, UnitType, Role } from '@realestate-crm/shared';
...
import { UnitStatusBadge, Badge } from '@/components/ui/Badge';
...
export const PropertiesPage: React.FC = () => {
  ...
  const [projectErrors, setProjectErrors] = useState<{ name?: string; location?: string }>({});
  const [buildingErrors, setBuildingErrors] = useState<{ name?: string; projectId?: string }>({});
  const [unitErrors, setUnitErrors] = useState<{ unitNumber?: string; price?: string; buildingId?: string }>({});

  // Query Projects
  const { data: projectsData, isLoading: isProjectsLoading } = useQuery<{ projects: ProjectDTO[] }>({
    queryKey: ['projects'],
    queryFn: () => apiClient<{ projects: ProjectDTO[] }>('/api/properties/projects'),
  });

  // Query Units
  const { data: unitsData, isLoading: isUnitsLoading } = useQuery<{ units: UnitDTO[] }>({
    queryKey: ['units', selectedProjectId, selectedBuildingId, selectedStatus, selectedType],
    queryFn: () => {
      ...
      return apiClient<{ units: UnitDTO[] }>(`/api/properties/units?${params.toString()}`);
    },
  });
  ...
  const createProjectMutation = useMutation({
    ...
    onError: (err: any) => error(err.message || 'Failed to create project'),
  });
  ...
  const createBuildingMutation = useMutation({
    ...
    onError: (err: any) => error(err.message || 'Failed to add building'),
  });
  ...
  const createUnitMutation = useMutation({
    ...
    onError: (err: any) => error(err.message || 'Failed to add unit'),
  });
  ...
  const handleBuildingSubmit = (e: React.FormEvent) => {
    ...
    if (!buildingForm.projectId) errors.projectId = 'Please select a parent project.';
  ...
  const handleUnitSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const errors: { unitNumber?: string; price?: string; buildingId?: string } = {};
    if (!unitForm.buildingId) errors.buildingId = 'Please select a target building.';
    if (!unitForm.unitNumber.trim()) errors.unitNumber = 'Unit Number is required.';
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900 tracking-tight">Properties & Unit Inventory</h2>
          <p className="text-xs text-slate-500 mt-0.5">
            Project $\rightarrow$ Building $\rightarrow$ Unit hierarchy management and live availability matrix
          </p>
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-slate-500">{projects.length} Projects Configured</span>
        </div>
            ...
            {projects.map((project) => (
              ...
              >
                ...
                <p className="text-xs text-slate-600 mt-3 line-clamp-2 leading-relaxed">
                  {project.description || 'Modern residential development with high-end amenities.'}
                </p>

                <div className="flex items-center justify-between pt-4 mt-4 border-t border-slate-100 text-xs">
                  <span className="text-slate-500">
                    <strong className="text-slate-800">{project.buildingsCount || 0}</strong> Buildings
                  </span>
                  <div className="flex items-center gap-2">
                    <span className="text-emerald-700 font-semibold bg-emerald-50 px-2 py-0.5 rounded">
                      {project.availableUnits || 0} Available
                    </span>
                    <span className="text-slate-400">/ {project.totalUnits || 0} Units</span>
                  </div>
                </div>
              </Card>
            ))}
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <span className="text-xs text-slate-500">{units.length} Units Matching Filters</span>
        </div>
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{unit.building?.name}</p>
                  </div>
                ...
                {unit.booking && (
                  <div className="mt-3 p-2 bg-slate-100/80 rounded text-[11px] text-slate-600 flex items-center justify-between">
                    <span>Booked by:</span>
                    <strong className="text-slate-800">{unit.booking.lead?.name || 'Customer'}</strong>
                  </div>
                )}
              </Card>
            ))}
      ...
      >
        <form onSubmit={handleProjectSubmit} className="space-y-4" noValidate>
          ...
          <div className="flex justify-end gap-2 pt-3 border-t">
            <Button variant="outline" size="sm" onClick={() => setIsAddProjectModalOpen(false)}>
              Cancel
            </Button>
      ...
      >
        <form onSubmit={handleBuildingSubmit} className="space-y-4" noValidate>
          ...
          <div className="flex justify-end gap-2 pt-3 border-t">
            <Button variant="outline" size="sm" onClick={() => setIsAddBuildingModalOpen(false)}>
              Cancel
            </Button>
      ...
      >
        <form onSubmit={handleUnitSubmit} className="space-y-4" noValidate>
          ...
          <div className="flex justify-end gap-2 pt-3 border-t">
            <Button variant="outline" size="sm" onClick={() => setIsAddUnitModalOpen(false)}>
              Cancel
            </Button>
