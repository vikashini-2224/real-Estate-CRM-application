...
export const UsersPage = () => {
  ...
  const createUserMutation = useMutation({
    ...
    onError: (err) => error(err.message || "Failed to create user"),
  });
  ...
  const toggleActiveMutation = useMutation({
    ...
    onError: (err) => error(err.message || "Failed to update user status"),
  });
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-5 rounded-xl border border-slate-200/80 shadow-sm">
        <div>
          <h2 className="text-xl font-bold text-slate-900 tracking-tight">
            Staff & Sales Rep Management
          </h2>
          <p className="text-xs text-slate-500 mt-0.5">
            Configure system roles, access credentials, and representative
            capacity
          </p>
        </div>
        <Button
