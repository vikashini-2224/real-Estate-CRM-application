...
export const LeadsPage = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => {
      ...
      return apiClient(`/api/leads?${params.toString()}`);
    },
  });
  ...
  const handleCreateSubmit = (e) => {
    ...
    createLeadMutation.mutate({
      ...
      followUpDate: newLead.followUpDate
        ? new Date(newLead.followUpDate).toISOString()
        : null,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Manage customer inquiries, assign sales representatives, and log
            follow-ups
          </p>
        </div>
        <Button
      ...
      <Card className="p-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          >
            ...
            <span>
              {myLeadsOnly ? "Showing My Assigned Leads" : "Filter My Leads"}
            </span>
          </button>
        </div>
      </Card>
      ...
      <ConfirmDialog
        ...
        onConfirm={() =>
          leadToDelete && deleteLeadMutation.mutate(leadToDelete.id)
        }
        title="Delete Lead"
