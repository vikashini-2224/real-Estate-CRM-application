...
export const LeadDetailPage = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["lead", id],
    queryFn: () => apiClient(`/api/leads/${id}`),
  });

  // Query Available Units (for booking modal)
  const { data: unitsData } = useQuery({
    queryKey: ["available-units"],
    queryFn: () => apiClient("/api/properties/units?status=AVAILABLE"),
  ...
  const { data: buildingUnitsData } = useQuery({
    queryKey: ["units", editForm.interestedBuildingId],
    queryFn: () =>
      apiClient(
        `/api/properties/units?buildingId=${editForm.interestedBuildingId}`,
      ),
  ...
  const availableBuildings =
    projectsData?.projects.find((p) => p.id === editForm.interestedProjectId)
      ?.buildings || [];
  ...
  const bookUnitMutation = useMutation({
    ...
    onError: (err) => {
      error(
        err.message ||
          "Unit booking failed. It might have been booked by another user.",
      );
    },
  });
  ...
  const handleBookSubmit = (e) => {
    ...
    const errors = {};
    if (!selectedUnitId)
      errors.unitId = "Please select an available property unit.";
    if (!finalPrice) errors.finalPrice = "Agreed final price is required.";
    if (!bookingAmount)
      errors.bookingAmount = "Initial token amount is required.";
        ...
        <div className="w-full overflow-x-auto scrollbar-hide">
          <div className="flex w-[750px] lg:w-full items-start justify-between pt-1">
            {stagesList.map((st, idx) => {
              ...
              const isPassed = stagesList.indexOf(lead.stage) >= idx;
              ...
              switch (st) {
                case LeadStage.NEW:
                  subtitle = "Fresh inquiry";
                  break;
                case LeadStage.CONTACTED:
                  subtitle = "Initial contact";
                  break;
                case LeadStage.SITE_VISIT:
                ...
                case LeadStage.NEGOTIATION:
                  subtitle = "Price & terms";
                  break;
                case LeadStage.BOOKED:
                  subtitle = "Closed won";
                  break;
              }
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    disabled={
                      lead.stage === LeadStage.BOOKED && st !== LeadStage.BOOKED
                    }
              ...
              <div className="pt-4 border-t border-slate-100">
                ...
                lead.interestedUnit ? (
                  ...
                  <p className="text-[11px] text-slate-400 italic">
                    No specific property interest specified.
                  </p>
                )}
              </div>
            </CardContent>
              ...
              <CardContent className="space-y-3 text-xs">
                ...
                <div className="flex justify-between py-1 border-b border-emerald-100/80">
                  ...
                  <span className="font-bold text-slate-900">
                    {lead.booking.unit?.building?.name} — Unit #
                    {lead.booking.unit?.unitNumber}
                  </span>
                </div>
                <div className="flex justify-between py-1 border-b border-emerald-100/80">
                  <span className="text-slate-500">Unit Type:</span>
                  <span className="font-semibold text-slate-800">
                    {lead.booking.unit?.type}
                  </span>
                </div>
                <div className="flex justify-between py-1 border-b border-emerald-100/80">
            ...
            <CardHeader>
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Chronological history of interactions, calls, and meetings
                </p>
              </div>
            </CardHeader>
              ...
              >
                ...
                <div className="flex justify-between items-center">
                  <span className="text-[11px] text-slate-400">
                    Posting as:{" "}
                    <strong className="text-slate-600">{user?.name}</strong>
                  </span>
                  <Button
                    type="submit"
                    size="sm"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <Select
                ...
                onChange={(e) => {
                  setEditForm({
                    ...editForm,
                    interestedBuildingId: e.target.value,
                    interestedUnitId: "",
                  });
                }}
              ...
              <Select
                ...
                onChange={(e) =>
                  setEditForm({ ...editForm, interestedUnitId: e.target.value })
                }
        ...
        <form onSubmit={handleBookSubmit} className="space-y-4" noValidate>
          <div className="p-3 bg-blue-50 rounded-lg border border-blue-200 text-xs text-blue-800 space-y-1">
            ...
            <p className="text-[11px] leading-relaxed text-blue-700">
              When submitted, this unit is instantly locked at the database
              level. If another agent is attempting to book the same unit
              simultaneously, strict transaction isolation prevents collision.
            </p>
          </div>
          ...
          >
            ...
            {unitsData?.units?.map((u) => (
              <option key={u.id} value={u.id}>
                {u.building?.project?.name} &bull; {u.building?.name} &bull;
                Unit #{u.unitNumber} ({u.type}) — {formatCurrencyINR(u.price)}
              </option>
            ))}
          </Select>
