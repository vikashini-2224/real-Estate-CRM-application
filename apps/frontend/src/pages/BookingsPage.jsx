...
import { CalendarCheck, XCircle, ExternalLink } from "lucide-react";
...
export const BookingsPage = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["bookings"],
    queryFn: () => apiClient("/api/bookings"),
  });
  ...
  const cancelBookingMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      success(
        "Booking cancelled and property unit safely released back to available inventory.",
      );
    },
    onError: (err) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-5 rounded-xl border border-slate-200/80 shadow-sm">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Audit-backed transactions connecting verified leads with dedicated
            property units
          </p>
        </div>
        <div className="flex items-center gap-4 text-xs">
          <div className="text-right">
            ...
            <span className="font-extrabold text-slate-900 text-sm">
              {formatCurrencyINRCompact(totalVolume)}
            </span>
          </div>
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5">
                      ...
                      <p className="text-[11px] text-slate-500">
                        {b.unit?.building?.project?.name} &bull;{" "}
                    ...
                    <td className="px-5 py-3.5">
                      <p className="font-bold text-emerald-700">
                        {formatCurrencyINR(b.finalPrice)}
                      </p>
      ...
      <ConfirmDialog
        ...
        onConfirm={() =>
          bookingToCancel && cancelBookingMutation.mutate(bookingToCancel.id)
        }
