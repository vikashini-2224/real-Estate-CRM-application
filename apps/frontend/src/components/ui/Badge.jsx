...
export const LeadStageBadge = ({ stage, size = "md" }) => {
  const stageMap = {
  ...
  return (
    <Badge variant={config.variant} size={size}>
      <span
        className={cn("w-1.5 h-1.5 rounded-full inline-block", config.dotColor)}
      />
      {config.label}
    </Badge>
  );
...
export const UnitStatusBadge = ({ status, size = "md" }) => {
  const statusMap = {
